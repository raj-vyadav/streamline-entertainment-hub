...
import { Send, Smile, Users, LogIn, Lock } from "lucide-react";
...
const SocialChat = ({ partyId, contentId, partyStatus }: SocialChatProps) => {
  const { messages, sendMessage, loading, isAuthenticated, isLive } = useRealtimeChat({ partyId, contentId, partyStatus });
  const { user } = useAuth();
  ...
  const ended = partyStatus === "ended";
  const scheduled = partyStatus === "scheduled";
    ...
    <div className="flex flex-col h-full rounded-2xl glass overflow-hidden">
      <div className="flex items-center justify-between p-4 border-b border-border">
        ...
        <span className="flex items-center gap-1.5 text-xs text-primary">
          {isLive && <span className="h-1.5 w-1.5 rounded-full bg-destructive animate-pulse" />}
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-3 max-h-80">
        {loading ? (
          ...
          <p className="text-xs text-muted-foreground text-center py-4">
            {scheduled ? "Chat opens when the party goes live." : "No messages yet. Be the first!"}
          </p>
        ) : (
          <AnimatePresence>
            {messages.map((msg) => (
      ...
      <div className="p-3 border-t border-border">
        {ended ? (
          <div className="flex items-center justify-center gap-2 p-2 text-xs text-muted-foreground">
            ...
            This watch party has ended. Chat is closed.
          </div>
        ) : scheduled ? (
          <div className="flex items-center justify-center gap-2 p-2 text-xs text-muted-foreground">
            <Lock className="h-3.5 w-3.5" />
            Chat will open when the party goes live.
          </div>
        ) : isAuthenticated ? (
